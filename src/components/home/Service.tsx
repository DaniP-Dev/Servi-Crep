...
// Type definitions
type ServiceKey = 'Comercial' | 'Residencial' | 'Industrial';
...
// Content
const serviceTexts: ServiceTexts = {
  sectionTitle: 'Nuestros Servicios',
  sectionSubtitle: 'Inspección técnica acreditada para estaciones de servicio',
  sectionDescription:
    'Ofrecemos servicios de inspección imparcial y objetiva, garantizando el cumplimiento normativo y la seguridad en el almacenamiento y distribución de combustibles líquidos.',
  tabs: [
...
// Helper function to map label to service key
const getLabelKey = (label: string): ServiceKey => {
  const labelMap: Record<string, ServiceKey> = {
    Comercial: 'Comercial',
    Residencial: 'Residencial',
    Industrial: 'Industrial',
  };
  return labelMap[label] || 'Comercial';
};
...
const Service = () => {
  ...
  return (
    <div className="container-fluid service py-5" ref={carouselRef}>
        ...
        <div className="row g-5 align-items-center">
          ...
          <div className="col-lg-3 wow fadeInLeft" data-wow-delay="0.2s">
            <ul className="nav">
              {serviceTexts.tabs.map((tab, idx) => (
                <li
                  className={`nav-item me-lg-0 mb-4${idx === 2 ? ' mb-lg-0' : ''}`}
                  key={tab.id}
                >
                  <a
                    className={`py-3 px-5${idx === 0 ? ' active' : ''}`}
                    data-bs-toggle="pill"
                    href={`#ServiceTab-${tab.id}`}
                  >
                    <span>{tab.label}</span>
                  </a>
                </li>
          ...
          <div className="col-lg-9 wow fadeInRight" data-wow-delay="0.3s">
            <div className="tab-content">
              {serviceTexts.tabs.map((tab, idx) => {
                const key = getLabelKey(tab.label);
                return (
                  <div
